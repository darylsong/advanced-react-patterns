...
This pattern takes the previous pattern to the next stage by allowing the user
to **completely** control state values. Instead of allowing the user to only
pass in a `reducer` to manage the state of the component, this pattern allows
the user to pass in the state value directly.

Here, we custom component `Toggle` that implements the custom `useToggle` hook
for state management purposes. The `useToggle` hook allows the user to optionally pass in the `on` value. If the user so decides to pass in the `on` state value,
then the state of the `useToggle` hook will be controlled by the user. The user
should then also pass in an `onChange` function that allows the user to manage
the state of the component directly.

However, if the user does not wish to control the state of the component, the
user can simply omit passing any state value or `onChange` function to the
custom component.

The above pattern is similar to how React treats form elements. For example,
the `input` element can accept a `value` component. If so, then React
understands that the user wishes to control the state of the `input` element.
The user can then pass in an `onChange` function that will be called when the
state of the `input` element is changed.

More advanced forms of this pattern involve the introduction of useful
warnings to the user. For example, where the user provides a `value` component
but fails to provide an `onChange` function, the component will be read-only.
The component should then warn the user accordingly, unless the user explicitly
indicates that the component should indeed be read-only.
