...
This pattern may be used when we need to pass the same set of props to the
children of a particular React component. We can pass these props directly
to the children like this:

```jsx
<ChildrenComponent1 value1={value1} value2={value2} />
<ChildrenComponent2 value1={value1} value2={value2} />
<ChildrenComponent3 value1={value1} value2={value2} />
```

However, this way of passing props results in less flexibility. If we want
to pass on additional props to these children components in the future, we
will need to update all of the code above. This pattern aims to solve this
problem by wrapping all the children component in a wrapper component that
then uses the `React.Children.map` function to map each children component
to a new version of itself using the `React.cloneElement` function. The
cloned children component will then be passed the props, which is accepted
as the second argument of the `React.cloneElement` function.

In addition, if we want to pass on the props to only select children
components, we can also create a `allowedTypes` array which will contain a
list of types of components that we want to pass the props to. When calling
the `React.Children.map` function, we will then check if the child
component is included in the `allowedTypes` array.
