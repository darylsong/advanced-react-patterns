...
The previous pattern only allows for the passing of props to immediate children.
However, if we want to pass props beyond the immediate children of a particular
component, we should use the `React.createContext` API instead. We can also
create a custom hook to consume the context (in this example, `useToggle`).
