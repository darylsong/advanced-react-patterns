...
This pattern involves the creation of custom hooks that support common use
cases for UI elements. For example, a `button` component should typically
have an `onClick` event handler. In the custom hook, we can return an
object that contains the necessary props that we can pass directly to these
UI elements (see the `togglerProps` object in the example below).

A more advanced form of this pattern is to return a function rather than
an object that contains the necessary props (see the `getTogglerProps`
function below). This function can accept an `onClick` argument from the
user, which it will call before proceeding to perform its usual function.

Note also the creation of the `callAll` function, which accepts other
functions as arguments. It then returns a single function that when called
