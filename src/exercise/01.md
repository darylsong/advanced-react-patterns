...
This pattern relates to the `useContext` and `useReducer`hooks. It is common to
create a context provider component (see the component `CounterProvider` in the
example below) and a custom hook to consume the context (see the custom hook
`useCounter` in the example below).

However, a simple implementation of a `useContext` and `useReducer` combo requires
the user of the reducer to implement their own functions on which `dispatch` will
be called. This is not a good API. This pattern eliminates this problem by creating
helper functions that accept `dispatch` as an argument which it then uses to
dispatch functions.

Accepting `dispatch` as an argument allows for the function to be declared and
initialised outside of the context provider component (in this case,
`CounterProvider`). The helper function need not be declared within the context
provider component in order to have access to `dispatch`. This improves
performance and/or code readability.

Performance is improved as the helper function will not be re-created on each
render of the context provider component. It may be possible to improve performance
by wrapping the helper function in a `useCallback` hook. However, this is not
recommended as such helper functions are 'object junk'. This also produces worse
performance as React now has additional dependencies (in this case, `dispatch`) on
each render of the context provider component.
