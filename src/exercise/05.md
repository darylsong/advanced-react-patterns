...
It is common to create a custom hook that implements the `useReducer` hook
for state management. This pattern involves the passing of control over the
state management of such a hook to the user. A basic form of this pattern is
for the custom hook to accept a `reducer` as an argument, which it will then
use for state management (see the custom hook `useToggle` in the example
below).

However, a more advanced form of this pattern allows the user to optionally
provide a `reducer` (see the custom reducer `toggleStateReducer` in the
example below). If the user does not provide a `reducer`, the custom hook
will utilise a default `reducer` for state management (see the default
reducer `toggleReducer` in the example below).

Another enhancement to this pattern involves the custom `reducer` calling
the default `reducer` to handle default actions. This eliminates the need
for users to re-implement the default reducer.
